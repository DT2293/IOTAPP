...
const fetchData = async (deviceId) => {
  try {
    const [tempRes, humidRes, smokeRes] = await Promise.all([
      ...
      axios.get(
        `https://blynk.cloud/external/api/get?token=${BLYNK_TOKEN}&pin=V1`,
      ),
      axios.get(
        `https://blynk.cloud/external/api/get?token=${BLYNK_TOKEN}&pin=V3`,
      ),
    ]);
  ...
  ws.on("message", async (message) => {
    try {
      ...
      if (!ws.isAuthenticated) {
        ws.send(
          JSON.stringify({ type: "auth_error", message: "Bạn chưa xác thực!" }),
        );
        return;
      }
      ...
      if (!userDevices || !userDevices.devices.includes(data.deviceId)) {
        console.warn(
          `User ${ws.userId} không có quyền truy cập deviceId ${data.deviceId}`,
        );
        return;
      }
      ...
      if (data.action === "toggleRelay") {
        try {
          await axios.get(
            `https://blynk.cloud/external/api/update?token=${BLYNK_TOKEN}&pin=V0&value=${data.state === "on" ? 1 : 0}`,
          );
          ws.send(
            JSON.stringify({
...
const sendData = async () => {
  ...
  for (const user of users) {
    for (const deviceId of user.devices) {
      ...
      if (
        JSON.stringify(newData) !== JSON.stringify(previousData.get(deviceId))
      ) {
        if (newData.flame === 1) {
